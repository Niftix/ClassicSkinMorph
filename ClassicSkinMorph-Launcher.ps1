...
$detail.Text = 'Connexion a GitHub...'
...
function Receive-Update([string]$Uri, [string]$Destination) {
    ...
    $client.add_DownloadProgressChanged({
        ...
        $detail.Text = if ($_.TotalBytesToReceive -gt 0) {
            '{0:N1} / {1:N1} Mo' -f ($_.BytesReceived / 1MB), ($_.TotalBytesToReceive / 1MB)
        } else { '{0:N1} Mo recus' -f ($_.BytesReceived / 1MB) }
    })
    try {
...
function Install-Update([string]$CommitSha) {
    ...
    try {
        Set-LauncherStatus 'Telechargement de la mise a jour...' 'Preparation du telechargement'
        Receive-Update "https://codeload.github.com/$repository/zip/$CommitSha" $archive
        $progress.Style = 'Marquee'
        Set-LauncherStatus 'Installation de la mise a jour...' 'Extraction des fichiers'
        ...
        if (-not $source) { throw 'Archive GitHub invalide.' }
        foreach ($required in @('ClassicSkinMorph.ps1', 'ClassicSkinMorph.psm1', 'config.example.json')) {
            if (-not (Test-Path -LiteralPath (Join-Path $source.FullName $required) -PathType Leaf)) {
                throw "Fichier obligatoire absent : $required"
            }
        }
        Set-LauncherStatus 'Installation de la mise a jour...' 'Copie des fichiers'
...
$form.Add_Shown({
    try {
        if ($developmentCheckout -and -not $ForceUpdate) {
            Set-LauncherStatus 'Version de developpement locale.' 'Mise a jour automatique ignoree dans un depot Git'
            if (-not $NoLaunch) {
        ...
        Set-LauncherStatus 'Recherche de mises a jour...' 'Interrogation du depot GitHub'
        ...
        if ([string]::IsNullOrWhiteSpace($remoteSha)) { throw 'Version GitHub introuvable.' }
        ...
        if ($ForceUpdate -or -not (Test-Path -LiteralPath $application) -or $localSha -ne $remoteSha) {
            ...
            Set-LauncherStatus 'Mise a jour terminee.' 'Lancement de ClassicSkinMorph'
        } else { Set-LauncherStatus 'ClassicSkinMorph est a jour.' 'Lancement en cours' }
    } catch {
        if (-not (Test-Path -LiteralPath $application)) {
            [Windows.Forms.MessageBox]::Show("Telechargement impossible.`r`n`r`n$($_.Exception.Message)", 'ClassicSkinMorph', 'OK', 'Error') | Out-Null
            $form.Close()
            return
        }
        Set-LauncherStatus 'GitHub est indisponible.' 'Demarrage de la version locale'
    }
    if (-not $NoLaunch) {
        Start-Process powershell.exe -WorkingDirectory $installDirectory -ArgumentList @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', "`"$application`"") | Out-Null
    }
